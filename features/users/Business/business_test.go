...
//		userData.On("CheckUser", mock.AnythingOfType("user.UserCore")).Return(users.UserCore{}, errors.New("error check data")).Once()
//		data, err := userBusiness.LoginUser(userLogin)
//		assert.Equal(t, "error check data", err.Error())nil
//		assert.NotNil(t, err)
...
func TestUserStartCourse(t *testing.T) {
	t.Run("success start course ", func(t *testing.T) {
		...
		_, _, status := userBusiness.UserStartCourse(context.Background(), "123" , riza.ID)
